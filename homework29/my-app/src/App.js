    ...
    return (
      <div className="App">
            ...
            <div className='section'>
                <p>Choose anyone emoji who do you want {String.fromCodePoint('9193')}.</p> 
                <button onClick={this.handlerShowEmojis}>Show emoji</button>
        </div>
        <div className="Emoji">
            {this.state.isOpen ? 
              (this.state.emojis.map((item, index) => {
                return (<Emoji key={index} unicode={item.unicode} onClick={this.handlerOnClick.bind(this, index) } />)
                })) :
                null
            }
        </div>
           
        <button onClick={this.handlerResults}>Show Results</button>
